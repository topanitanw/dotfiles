...
# Usage: p4_reopen_to_cl <changelist_number>
p4_reopen_to_cl() {
    local clno=$1
    ...
    if [ -z "$clno" ]; then
        ...
        echo "Usage: p4_reopen_to_cl <changelist_number>"
        return 1
    fi
...
p4_set_root() {
    ...
    local client_root=$(echo "$p4_info_output" | grep "^Client root:" | cut -d ":" -f 2- | sed 's/^[[:space:]]*//')
...
p4_set_client() {
    ...
    local client_name=$(echo "$p4_info_output" | grep "^Client name:" | cut -d ":" -f 2- | sed 's/^[[:space:]]*//')
